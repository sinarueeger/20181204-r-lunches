...
- **rOpenSci Community call**: *Code Review in the Lab* `r Citep(references, "ropensci-2018")` and the [summary](https://ropensci.org/blog/2018/11/29/codereview/) of it.

- Plus all the [references](#references) listed throughout the presentation.
...
.small[*Enhancing reproducibility for computational methods* `r Citep(references, "stodden-etal-2016")`]
...
tail(dat)
dim(dat)
dat.qc <- janitor::remove_empty(dat)
dim(dat.qc)
...
### Identify duplicates row wise (2/3)

```{r}
dat.qc.nodupl <- janitor::get_dupes(dat.qc, id)
dat.qc.nodupl
```
...
### Consistent variable names (3/3)


```{r}
dat.qc.nodupl %>% names()
janitor::clean_names(dat.qc.nodupl, case = "snake") %>% names()
janitor::clean_names(dat.qc.nodupl, case = "lower_camel") %>% names()
```
...
source("src/B_fit.R")
rmarkdown::render("reports/report-intro.Rmd")
rmarkdown::render("reports/report-analyis.Rmd")
```
...
Rscript --vanilla -e "rmarkdown::render(\"reports/report-intro.Rmd\")"
Rscript --vanilla -e "rmarkdown::render(\"reports/report-analyis.Rmd\")"
```
...
# 2. Use gist.github.io

to memorise stuff, see https://gist.github.com/sinarueeger/ee42a74cbcb1d834e906199b26bc19f0
...
- [**Packaging Data Analytical Work Reproducibly Using R (and Friends)**](https://www.tandfonline.com/doi/full/10.1080/00031305.2017.1375986)

- [**An introduction to rrtools**](https://benmarwick.github.io/UO-2018-On-Ramps-to-Reproducibility/UO-2018-On-Ramps-to-Reproducibility.html#1)
